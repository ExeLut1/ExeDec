...
def Enam():
        ...
        os.rename(ff, nm)
        run('file berhasil di decrypted Ke %s' % nm)
def Tujuh():
        clr()
        banner_dec()()
        try:
...
def unmarsh():
    ...
    print h + '\nMenu ' + p + ':\n [' + h + '1' + p + ']. Mengecek Versi Script\n [' + h + '2' + p + ']. Kembali Ke Menu'
    try:
        ...
        cek = 1
        try:
            print h + '[' + k + '#' + h + ']' + p + ' Contoh : /path/marsh.py'
...
def pyc():
    ...
    if ask == 'y' or ask == 'Y':
        ...
        raw_input(m + '[' + p + '^' + m + ']' + p + ' Tekan Enter untuk Kembali ke menu ')
        os.system('clear')
        menu()
def unpyc():
...
def mzb():
    ...
    print('\x1b[31;1m[\x1b[0;37m+\x1b[31;1m]\x1b[0;37m File Di Simpan as\x1b[32;1m ' + d)
    print('Mau Pakai Lagi? (Y/N) ?')
...
def zl():
    ...
    f = open(d, 'w')
    f.write('# Berhasil Decompile ✓ \n'+ e)
    f.close()
    os.system('rm -rf ma.py')
    print('File Di Simpan '+ d)
    sys.exit()
def exit():
        run('MAKASIH ANJING DAH PAKE :)')
        sys.exit()
